...
GLSLLexer::GLSLLexer(QObject *parent)
        :QsciLexerCPP(parent)
{
...
QString GLSLLexer::description(int ) const
{
...
{
    if (set == 1)
        return "float int bool vec2 vec3 vec4 ivec2 ivec3 ivec4 bvec2 bvec3 bvec4 mat2 mat3 mat4 sampler1D sampler2D sampler3D sampler2DShadow sampler3DShadow samplerCube";
    if (set == 5)
        return "gl_MaxVertexAttribs   gl_MaxVertexUniformComponents   gl_MaxVaryingFloats "
               ...
               "gl_Fog";
    if (set == 4)
        return "gl_VertexID   gl_InstanceID   gl_Position   gl_PointSize "
    ...
    if (set == 3)
        return "attribute const in inout out uniform varying invariant "
               "centroid flat smooth noperspective layout patch sample "
               "subroutine lowp mediump highp precision";
    if (set == 2)
        return "radians degrees sin cos tan asin acos atan sinh "
